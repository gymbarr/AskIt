  ...
  describe '#notify_user_about_new_answer' do
    ...
    context 'when locale set to en' do
      it 'renders the subject for en locale' do
        I18n.locale = :en
        expect(mail.subject).to eq("You've got a new reply to your question! | AskIt")
      end

      it 'renders the body for en locale' do
        I18n.locale = :en
        expect(mail.body.encoded).to match('left an answer to your question:')
      end
    end
    ...
    context 'when locale set to ru' do
      it 'renders the subject for ru locale' do
        I18n.locale = :ru
        expect(mail.subject).to eq('Вам оставили ответ на Ваш вопрос! | AskIt')
      end
